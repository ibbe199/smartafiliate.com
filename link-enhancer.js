(function(){
  ...
  const INDEX_PAGES=new Set(['articles.html','ai-articles.html','posts-ai.html']);
  const RELATED=[
  ...
  function fixExternalLinks(){document.querySelectorAll('a[href^="http"]:not([href*="smartafiliate.com"])').forEach(a=>{a.target='_blank';a.rel='noopener noreferrer nofollow';});}
  function run(){const current=norm();if(EXCLUDED_PAGES.has(current)){fixExternalLinks();return;}addStyle();enhanceCards();injectArticleLinks();fixExternalLinks();setTimeout(enhanceCards,900);}
