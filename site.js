...
function loadLinkEnhancer(){
  if(document.getElementById('link-enhancer-js'))return;
  const s=document.createElement('script');
  s.id='link-enhancer-js';
  s.src='/link-enhancer.js?v=20260501_links_google';
  s.defer=true;
  document.body.appendChild(s);
}
...
function loadFooterManager(){
  if(document.getElementById('footer-manager-js'))return;
  const s=document.createElement('script');
  s.id='footer-manager-js';
  s.src='/footer-manager.js?v=20260501_full_footer';
  s.defer=true;
  document.body.appendChild(s);
}
...
function init(){loadCss();hardClean();buildHeader();optimizeImages();requestAnimationFrame(()=>{loadAllArticles();loadLinkEnhancer();loadMonetizationManager();loadFooterManager();addAiFeatured();buildFooter();finalPass();setTimeout(loadFooterManager,1200);});setTimeout(finalPass,900);}
