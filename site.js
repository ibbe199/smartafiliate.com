...
function loadCss(){let old=document.getElementById('header-fix-css');if(old)old.remove();const l=document.createElement('link');l.id='header-fix-css';l.rel='stylesheet';l.href='/header-fix.css?v=20260501m';document.head.appendChild(l);}
...
function removeAllOldFooters(){document.querySelectorAll('footer,.site-footer,.smart-global-footer').forEach(e=>e.remove());}
function removeHugeLogoImages(){
  document.querySelectorAll('main img, body > img, .hero img, .page-hero img').forEach(img=>{
    const src=(img.getAttribute('src')||'').toLowerCase();
    if(src.includes('/assets/images/icons/logo.png') || src.endsWith('logo.png')) img.remove();
  });
}
...
function init(){
  ...
  loadMenuScript();
  addFinalFooter();
  setTimeout(removeHugeLogoImages,300);
  lockMobile();
}
