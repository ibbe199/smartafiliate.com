(function () {
  ...
  const pages = {
    'index.html': {title:'Smart Affiliate', badge:'Home', intro:'قائمة الصفحة الرئيسية.', mode:'home'},
  ...
  const homeList = [
    ['learn-ai.html','تعلم AI','Learning'],
    ['best-ai-tools.html','أفضل أدوات AI','Tools'],
    ['ai-articles.html','مكتبة AI','AI Library'],
    ['open-source.html','أدوات مفتوحة المصدر','Open Source'],
    ['articles.html','المقالات العامة','Articles'],
    ['posts-ai.html','كل مقالات AI','All Posts']
  ];
  ...
  function itemsFor(cfg){
    if(!cfg) return [];
    if(cfg.mode === 'home') return homeList;
    if(cfg.mode === 'all') return items;
    if(cfg.mode === 'posts') return items.filter(i => i[0].startsWith('posts-ai/'));
    return items.filter(i => cfg.categories.includes(i[2]));
  }
  ...
  function addStyles(){
    ...
    s.id = 'route-css';
    s.textContent = `
      .site-header .logo{display:flex!important;align-items:center!important;gap:.55rem!important;text-decoration:none!important;min-width:130px!important}
      .site-logo-img{display:block!important;width:54px!important;height:54px!important;object-fit:contain!important;border-radius:12px!important;background:#fff!important;padding:2px!important}
      .site-logo-word{font-weight:900!important;font-size:1rem!important;color:#fff!important;line-height:1!important}
      .main-nav a.active{color:#fff!important;background:rgba(234,88,12,.95)!important;border-radius:999px!important;padding:.55rem .85rem!important}
      .page-footer-list-section{background:#eef4fb;padding:2rem 0;border-top:1px solid #d9e4f2}
      .page-footer-list-box{background:#fff;border:1px solid #d9e4f2;border-radius:22px;padding:1rem;box-shadow:0 12px 28px rgba(15,23,42,.08)}
      .page-footer-list-title{text-align:center;margin:0 0 1rem;color:#0b1f3a;font-size:1.25rem}
      .page-footer-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:.75rem}
      .page-footer-link{display:flex;justify-content:space-between;align-items:center;gap:.6rem;background:#f8fafc;border:1px solid #e2e8f0;border-radius:15px;padding:.85rem .95rem;text-decoration:none!important;color:#0b1f3a!important;font-weight:900}
      .page-footer-link span{color:#ea580c;font-size:.72rem;font-weight:900}
      ...
      @media(max-width:760px){.site-logo-img{width:40px!important;height:40px!important}.site-logo-word{font-size:.82rem!important}.page-footer-list{grid-template-columns:1fr}}
    `;
  ...
  function normalizeHeader(){
    document.querySelectorAll('.site-header .logo').forEach(el=>{
      ...
      el.innerHTML = `<img class="site-logo-img" src="${LOGO}" alt="Smart Affiliate logo"><span class="site-logo-word">Smart Affiliate</span>`;
    });
    document.querySelectorAll('.main-nav').forEach(nav=>{
      ...
      const p=currentPage();
      nav.querySelectorAll('a').forEach(a=>{ if(a.getAttribute('href')===p) a.classList.add('active'); });
    });
  ...
  function footerList(list, title){
    return `<section class="page-footer-list-section"><div class="container"><div class="page-footer-list-box"><h2 class="page-footer-list-title">${esc(title)}</h2><div class="page-footer-list">${list.map(i=>`<a class="page-footer-link" href="${esc(i[0])}">${esc(i[1])}<span>${esc(i[2])}</span></a>`).join('')}</div></div></div></section>`;
  }

  function card(i){
    const catPage = categoryPages[i[2]] || 'ai-articles.html';
  ...
  function renderRoutedPage(){
    const p = currentPage();
    const cfg = pages[p];
    if(!cfg || cfg.mode === 'home') return;
    const main = document.querySelector('main');
    if(!main) return;
    const list = itemsFor(cfg);
    main.innerHTML = `<section class="page-hero"><div class="container"><span class="page-badge">${esc(cfg.badge)}</span><h1>${esc(cfg.title)}</h1><p>${esc(cfg.intro)}</p></div></section><section class="section"><div class="container"><div class="section-header"><span class="section-tag">${list.length} روابط</span><h2>المحتوى المناسب لهذه الصفحة</h2></div><div class="articles-grid">${list.map(card).join('')}</div></div></section>${footerList(list,'قائمة هذه الصفحة')}`;
  }

  function renderHomeFooterList(){
    const p=currentPage();
    if(p !== 'index.html' && location.pathname !== '/') return;
    document.querySelectorAll('.home-full-index,.page-footer-list-section').forEach(e=>e.remove());
    const main = document.querySelector('main');
    if(main) main.insertAdjacentHTML('beforeend', footerList(homeList,'قائمة الصفحة الرئيسية'));
  }
