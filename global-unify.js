(function(){
  ...
  function addStyle(){
    ...
    const css = `
      ...
      .tools-grid .tool-card,.articles-grid .article-card{min-height:260px!important;height:260px!important;display:flex!important;flex-direction:column!important;justify-content:space-between!important;align-items:center!important;text-align:center!important;padding:1rem!important}
      .home-card-visual,.article-image,.post-image,.card-image,.tool-image,.featured-image,.unified-visual{height:92px!important;min-height:92px!important;max-height:92px!important;width:100%!important;border-radius:18px!important;margin:0 0 .8rem!important;display:flex!important;align-items:center!important;justify-content:center!important;background:radial-gradient(circle at 18% 18%,rgba(255,255,255,.24),transparent 28%),linear-gradient(135deg,var(--sa-dark),var(--sa-blue) 60%,var(--sa-orange))!important;color:#fff!important;overflow:hidden!important}
      .home-card-visual span,.unified-visual span{width:48px!important;height:48px!important;border-radius:16px!important;background:rgba(255,255,255,.15)!important;border:1px solid rgba(255,255,255,.18)!important;display:flex!important;align-items:center!important;justify-content:center!important;font-size:1.45rem!important;color:#fff!important}
      ...
      .unified-actions{display:flex!important;flex-wrap:wrap!important;gap:.45rem!important;align-items:center!important;justify-content:center!important;margin-top:auto!important;width:100%!important}
      ...
      @media(max-width:760px){.tools-grid .tool-card,.articles-grid .article-card{height:230px!important;min-height:230px!important;min-width:78%!important}.home-card-visual,.article-image,.post-image,.card-image,.tool-image,.featured-image,.unified-visual{height:78px!important;min-height:78px!important;max-height:78px!important}.sa-btn,.sa-btn-primary,.sa-btn-secondary,.tool-link,.read-more{min-height:32px!important;font-size:.7rem!important;padding:.4rem .68rem!important}}
    `;
  ...
  function normalizeCardVisual(card, index){
    if(card.querySelector('.home-card-visual,.unified-visual,.article-image,.post-image,.card-image,.tool-image,.featured-image')) return;
    const icons = ['🤖','📚','🎓','🧩','⚙️','🔎','✍️','🚀'];
  ...
  function normalizeButtons(card){
    if(card.dataset.unifiedButtons === 'true') return;
    const key = cardKey(card);
    let internal = null;
    let external = null;
    if(key){ internal = toolMap[key][0]; external = toolMap[key][1]; }
    ...
    existing.forEach(a => {
      if(a.closest('.main-nav,.header-actions,.top-conversion-bar')) return;
  ...
  function fixLinks(){
    document.querySelectorAll('a[href]').forEach(a => {
      ...
      if(href.startsWith('http')){
        ...
        if(href.includes('hop.clickbank.net') && !rel.includes('sponsored')) rel.push('sponsored');
